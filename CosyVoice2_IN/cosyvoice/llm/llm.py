...
class TransformerLM(torch.nn.Module):
    def __init__(
        ...
        # 使用标准的交叉熵损失替代LabelSmoothingLoss
        self.criterion_ce = nn.CrossEntropyLoss(ignore_index=IGNORE_ID)
...
class Qwen2LM(TransformerLM):
    ...
    def inference_bistream(
            ...
            kv_cache_dict: Optional[Dict] = None,
        ...
        out_tokens = []
        # 支持跨调用的 KV-cache 复用
        if uuid is not None and kv_cache_dict is not None and reuse_cache and uuid in kv_cache_dict and kv_cache_dict[uuid] is not None:
            # 复用已有的 KV-cache
            cache = kv_cache_dict[uuid]
            logging.info(f'复用 LLM KV-cache for uuid={uuid}, cache shape: {cache[0][0].shape if cache is not None and len(cache) > 0 else "None"}')
            # 注意：当复用缓存时，lm_input 需要从上次的最后一个 speech token 继续
            # 但这里我们重新开始处理新的文本，所以 lm_input 仍然从 sos_eos_emb 开始
            # KV-cache 会自动处理上下文连续性
        else:
            # 首次调用或不需要复用，初始化新的 cache
            cache = None
            if uuid is not None and kv_cache_dict is not None:
                # 初始化新的 KV-cache 条目
                kv_cache_dict[uuid] = None
                logging.info(f'初始化新的 LLM KV-cache for uuid={uuid}')
        ...
        for this_text in text:
            ...
            while prompt_speech_token_emb.size(1) != 0:
                if text_cache.size(1) >= self.mix_ratio[0]:
                    ...
                    logging.debug('append {} text token {} speech token'.format(lm_input_text.size(1), lm_input_speech.size(1)))
            ...
            if prompt_speech_token_emb.size(1) == 0:
                if (len(out_tokens) != 0 and out_tokens[-1] == self.speech_token_size + 2) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
                    logging.debug('get fill token, need to append more text token')
                    if text_cache.size(1) >= self.mix_ratio[0]:
