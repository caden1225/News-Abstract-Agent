...
class CosyVoice:
    ...
    def inference_sft(self, tts_text, spk_id, stream=False, speed=1.0, text_frontend=True):
        logging.info(f"spk_id: {spk_id} from inference_sft")
...
class CosyVoice2(CosyVoice):
    ...
    def inference_sft_chunked(self, text_chunks, spk_id, stream=False, speed=1.0, text_frontend=True,
                             token_hop_len=None, mel_cache_len=None, **kwargs):
        ...
        try:
            ...
            def text_token_generator():
                ...
                for chunk_idx, text_chunk in enumerate(text_chunks if not isinstance(text_chunks, Generator) else text_chunks):
                    ...
                    logging.debug('processing chunk {}: {}'.format(chunk_idx, normalized_text))
            ...
            # 支持通过 kwargs 传递自定义 UUID 和其他参数
            tts_kwargs = {}
            if 'custom_uuid' in kwargs:
                tts_kwargs['custom_uuid'] = kwargs['custom_uuid']
            if 'reuse_cache' in kwargs:
                tts_kwargs['reuse_cache'] = kwargs['reuse_cache']
            if 'is_final' in kwargs:
                tts_kwargs['is_final'] = kwargs['is_final']
            
            start_time = time.time()
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed, **tts_kwargs):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
                start_time = time.time()
