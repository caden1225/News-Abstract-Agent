...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        try:
            ...
            try:
                # 设置 WETEXT_HOME 环境变量，使用项目中的本地 Wetext 资源
                # 避免从 ModelScope 下载（可能卡住）
                if 'WETEXT_HOME' not in os.environ:
                    ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
                    # frontend.py 在 cosyvoice/cli/ 目录下，Wetext 在 cosyvoice2/ 目录下
                    # 所以路径是: cosyvoice/cli/ -> cosyvoice/ -> cosyvoice2/ -> Wetext
                    wetext_local_path = os.path.abspath(os.path.join(ROOT_DIR, '../../Wetext'))
                    if os.path.exists(wetext_local_path):
                        os.environ['WETEXT_HOME'] = wetext_local_path
                        logging.info(f'use local wetext resources: {wetext_local_path}')
                ...
                self.text_frontend = ''
                logging.warning(f'no frontend is avaliable: {e}')
