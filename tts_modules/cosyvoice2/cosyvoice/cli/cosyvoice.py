...
class CosyVoice:
    ...
    def inference_zero_shot(self, tts_text, prompt_text, prompt_wav, zero_shot_spk_id='', stream=False, speed=1.0, text_frontend=True):
        if self.__class__.__name__ == 'CosyVoice3' and isinstance(tts_text, str) and '<|endofprompt|>' not in prompt_text + tts_text:
...
class CosyVoice2(CosyVoice):
    ...
    def inference_sft_chunked(self, text_chunks, spk_id, stream=True, speed=1.0, text_frontend=True, 
                              token_hop_len=None):
        """
        流式分块推理方法，支持文本块作为Generator输入
        
        Args:
            text_chunks: 文本块列表或生成器，每个块会被独立归一化但作为连续流处理
            spk_id: 说话人ID
            stream: 是否启用流式输出音频
            speed: 语速倍数（流式模式下不支持speed != 1.0）
            text_frontend: 是否进行文本归一化
            token_hop_len: 输出chunk大小（可选，默认使用模型默认值25）
            
        Yields:
            dict: 包含'tts_speech'键的字典，值为torch.Tensor音频数据
        """
        # 设置模型参数（如果提供了）
        ...
        try:
            # 对每个文本块进行归一化和tokenization，转换为token tensor的生成器
            # 注意：inference_bistream期望的是token tensor的Generator，每个元素是[1, N]的tensor
            # 直接使用传入的text_chunks（可以是生成器），实现真正的流式处理
            def token_tensor_generator():
                # 如果text_chunks是生成器，直接迭代；如果是列表/元组，也直接迭代
                # 这样可以在收到第一个chunk后立即开始处理，不需要等待所有chunk
                for chunk in text_chunks:
                    if text_frontend:
                        # 对每个chunk进行归一化
                        normalized = self.frontend.text_normalize(chunk, split=False, text_frontend=True)
                        if isinstance(normalized, list):
                            for text in normalized:
                                if text and text.strip():
                                    # Tokenize文本
                                    text_token = self.frontend.tokenizer.encode(text, allowed_special=self.frontend.allowed_special)
                                    text_token = torch.tensor([text_token], dtype=torch.int32).to(self.frontend.device)
                                    yield text_token
                        elif normalized and normalized.strip():
                            # Tokenize文本
                            text_token = self.frontend.tokenizer.encode(normalized, allowed_special=self.frontend.allowed_special)
                            text_token = torch.tensor([text_token], dtype=torch.int32).to(self.frontend.device)
                            yield text_token
                    else:
                        if chunk and chunk.strip():
                            # Tokenize文本
                            text_token = self.frontend.tokenizer.encode(chunk, allowed_special=self.frontend.allowed_special)
                            text_token = torch.tensor([text_token], dtype=torch.int32).to(self.frontend.device)
                            yield text_token
            ...
            # 构建模型输入
            # 注意：text参数传递token tensor的生成器，模型会识别Generator类型并调用llm.inference_bistream
            # inference_bistream期望的是token tensor的Generator，每个元素是[1, N]的tensor
            model_input = {
                'text': token_tensor_generator(),  # Token tensor的Generator
                'text_len': torch.tensor([0], dtype=torch.int32).to(self.frontend.device),  # 占位符，Generator模式下不使用
                'llm_embedding': embedding,
                'flow_embedding': embedding
            }
            
            # 调用模型进行推理
            # 模型内部会检查isinstance(text, Generator)，如果是Generator则调用llm.inference_bistream
            start_time = time.time()
            logging.info('synthesis chunked text with streaming input')
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
...
class CosyVoice3(CosyVoice2):
    ...
    def inference_sft_chunked(self, text_chunks, spk_id, stream=True, speed=1.0, text_frontend=True, 
                              token_hop_len=None):
        """
        CosyVoice3 流式分块推理方法，支持文本块作为Generator输入
        ...
        try:
            ...
            # 调用模型进行推理
            start_time = time.time()
            logging.info('CosyVoice3 synthesis chunked text with streaming input')
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
