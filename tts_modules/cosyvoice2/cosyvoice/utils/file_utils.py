...
def load_wav(wav, target_sr, min_sr=16000):
    # 尝试使用 soundfile 库直接加载（避免 torchcodec 兼容性问题）
    try:
        import soundfile as sf
        speech_array, sample_rate = sf.read(wav)
        speech = torch.from_numpy(speech_array).float()
        # 如果是立体声，转换为单声道
        if speech.dim() > 1:
            speech = speech.mean(dim=-1)
        speech = speech.unsqueeze(0)  # 添加 batch 维度
    except (ImportError, RuntimeError, Exception) as e:
        # 如果 soundfile 不可用或失败，尝试使用 torchaudio
        # 使用 sox_io 后端，避免 torchcodec 问题
        try:
            speech, sample_rate = torchaudio.load(wav, backend='sox_io')
        except Exception:
            # 最后尝试默认后端
